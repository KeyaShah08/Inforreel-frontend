...
function VerifyAccount() {
  ...
  useEffect(() => {
    ...
    const handleInitialResend = async () => {
      ...
      try {
        const response = await fetch("http://54.173.98.4:8000/api/users/resend-otp", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://54.173.98.4:8000/api/users/verify-otp", {
        method: "POST",
        headers: {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const resendApiUrl = "http://54.173.98.4:8000/api/users/resend-otp";
